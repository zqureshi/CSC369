...
sfs_creat(struct vnode *v, const char *name, int excl, struct vnode **ret)
{
	...
	/* Didn't exist - crv-> = sfs_makeobj(sfs, SFS_TYPE_FILE, &newguy);
	if (result) {
		lock_release(sv->sv_lock);
		return result;
	}
...
sfs_mkdir(struct vnode *v, const char *name)
{
   ...
   struct sfs_dir sfd[3];
   
   struct sfs_vnode * ret;
   
   int result;

   result = sfs_makeobj(v->vn_fs->fs_data, SFS_TYPE_DIR, &ret);
   
   if(result) return result;
   
   strcpy(sfd[2].sfd_name , name);
   sfd[2].sfd_ino = ret->sv_ino; 
   int num_slots; 
   num_slots = sfs_dir_nentries(v->vn_data);
   //check for limit of files that can be stuffed

   if ((size_t)num_slots == (((struct sfs_vnode *) v->vn_data)->sv_i.sfi_size / sizeof(struct sfs_dir)))
     return ENOSPC;
   sfs_writedir(v->vn_data, &sfd[2], num_slots + 1);
    
   
   sfd[0].sfd_ino = ret->sv_ino;
   strcpy(sfd[0].sfd_name, ".");
   sfd[1].sfd_ino = ((struct sfs_vnode *) v->vn_data)->sv_ino;
   strcpy(sfd[1].sfd_name, "..");
  
  sfs_writedir(ret, &sfd[0], 0);
  sfs_writedir(ret, &sfd[1], 1);
