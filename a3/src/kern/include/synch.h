...
struct lock {
	char *name;
	// owner is NULL if the lock is free, else thread that owns it
	// Note the use of volatile here: this says that the pointer variable
	// itself is volatile.  If we wanted instead a
	// pointer to a volatile thread struct, we would use
