...
#include <vm.h> /* ASST2: for vm_printstats function */

#if OPT_SYNCHPROBS
#include <lunchcounter.h>
#endif
...
{
	int result;
	pid_t childpid;  /* ASST1: To wait for thread running command */
        int childstatus; /* ASST1: get result from thread running command */
	...
	result = thread_fork(args[0] /* thread name */,
			...
			cmd_progthread, &childpid);
	if (result) {
		kprintf("thread_fork failed: %s\n", strerror(result));
		return result;
	}

        result = thread_join(childpid, &childstatus);
        if (result) {
...
cmd_quit(int nargs, char **args)
{
	...
	thread_exit(0); /* A0 SOLUTION: Added exit code */
	return 0;
}
...
#if !OPT_DUMBVM
	/* New tests for ASST2 */
	"[km3] Coremap alloc test            ",
	"[km4] Coremap alloc stress test     ",
	/* End new tests for ASST2 */
#endif
	"[tt1] Thread test 1                 ",
	"[tt2] Thread test 2                 ",
	"[tt3] Thread test 3                 ",
...
static const char *mainmenu[] = {
	...
	/* None defined yet */
        "[lc] Lunch counter sync problem     ",
#endif
        "[vm] Virtual memory stats           ", /* ASST2 */
...
#if OPT_SYNCHPROBS
	(void)item_time; /* Shut up compiler */
#endif
